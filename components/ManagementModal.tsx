import { X, Trash2, Plus, ShieldCheck, Wallet, Banknote, CalendarDays, ChevronDown, UserPlus, Users, History } from "lucide-react";
...
export default function ManagementModal({ activeTab, onClose, bookings, setBookings, servicesList, setServicesList, roomClasses, setRoomClasses, combos, setCombos, cashflow, setCashflow, permissions, setPermissions, activeBranch, role }: any) {
  ...
  const togglePermission = async (key: string, dbKey: string) => {
    ...
    if(branchId) await supabase.from('branch_permissions').update({ [dbKey]: newVal }).eq('branch_id', branchId);
  };
  ...
  // STATE MỚI CHO BẢNG LỊCH SỬ XÓA
  const [deletionLogs, setDeletionLogs] = useState<any[]>([]);

  const fetchReceptionAccounts = async () => {
    if (!branchId) return;
    const { data } = await supabase.from('users').select('*').eq('branch_id', branchId).eq('role', 'reception');
    if (data) setReceptionList(data);
  };

  // KÉO DỮ LIỆU TỪ "CUỐN SỔ NAM TÀO" XUỐNG
  const fetchDeletionLogs = async () => {
    if (!branchId) return;
    const { data } = await supabase.from('deletion_logs').select('*').eq('branch_id', branchId).order('created_at', { ascending: false }).limit(100);
    if (data) setDeletionLogs(data);
  };

  useEffect(() => { 
      if (activeTab === 'permissions') fetchReceptionAccounts(); 
      if (activeTab === 'deletionHistory') fetchDeletionLogs();
  }, [activeTab, branchId]);

  const handleCreateReception = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newRecUser.trim() || !newRecPass.trim() || !branchId) return;
    ...
    await supabase.from('users').insert([{ username: newRecUser.trim(), password: newRecPass.trim(), role: 'reception', branch_id: branchId }]);
  ...
  const handleAddService = async () => {
    ...
    if(!branchId) return alert("Lỗi: Không xác định được Chi nhánh hiện tại!");
    
    const newSrv = { id: `srv-${Date.now()}`, branch_id: branchId, name: newServiceName.trim(), price: Number(newServicePrice) };
  ...
  const handleAddRoomClass = async () => {
    ...
    await supabase.from('room_types').insert([{ branch_id: branchId, name: upper }]);
  ...
  const handleDeleteRoomClass = async (rc: string) => {
    ...
    if(confirm(`Xóa hạng phòng "${rc}"?`)) {
      await supabase.from('room_types').delete().match({ branch_id: branchId, name: rc });
      setRoomClasses((prev: any) => prev.filter((c: string) => c !== rc));
    }
  };

  const [selectedRoomClassForCombo, setSelectedRoomClassForCombo] = useState('');
  const [draftCombos, setDraftCombos] = useState([{ name: '', hours: '', minutes: '0', price: '', weekendPrice: '' }]);
  const currentComboRoomClass = selectedRoomClassForCombo || (roomClasses[0] || "");
  const handleAddDraftRow = () => setDraftCombos([...draftCombos, { name: '', hours: '', minutes: '0', price: '', weekendPrice: '' }]);
  ...
  const handleSaveBulkCombos = async () => {
    ...
    const newCombos = validDrafts.map(d => ({ 
        branch_id: branchId, 
        room_type: currentComboRoomClass, 
        name: d.name.trim().toUpperCase(), 
        hours: Number(d.hours || 0) + Number(d.minutes) / 60, 
        price: Number(d.price),
        weekend_price: Number(d.weekendPrice || d.price)
    }));
    await supabase.from('room_combos').insert(newCombos);
    
    const uiCombos = newCombos.map(c => ({ name: c.name, hours: c.hours, price: c.price, weekendPrice: c.weekend_price }));
    setCombos((prev: any) => ({ ...prev, [currentComboRoomClass]: [...(prev[currentComboRoomClass] || []), ...uiCombos] }));
    setDraftCombos([{ name: '', hours: '', minutes: '0', price: '', weekendPrice: '' }]);
  };
  const handleDeleteCombo = async (roomClass: string, comboName: string) => {
    if(!branchId) return;
    if(confirm(`Xóa "${comboName}"?`)) {
      await supabase.from('room_combos').delete().match({ branch_id: branchId, room_type: roomClass, name: comboName });
  ...
  const handleAddCashflow = async () => {
    ...
    const newCf = { id: `cf-${Date.now()}`, branch_id: branchId, date: cfDate, type: cfType, method: cfMethod, amount: Number(cfAmount), note: cfNote.trim() };
    ...
    <div className="fixed inset-0 bg-slate-900/80 z-[60] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-7xl h-[90vh] flex flex-col overflow-hidden">
        <div className="bg-slate-900 p-4 flex justify-between items-center text-white shrink-0">
            <h2 className="font-black text-lg flex items-center gap-2 uppercase tracking-wider">
                {activeTab === 'services' && '☕ Kho Dịch Vụ'} 
                {activeTab === 'rooms' && '🏨 Hạng Phòng'} 
                {activeTab === 'combos' && '🏷️ Combo Phòng'} 
                {activeTab === 'cashflow' && '💵 Sổ Quỹ Thu - Chi'} 
                {activeTab === 'stats' && '📊 Thống Kê'} 
                {activeTab === 'permissions' && '🛡️ Phân Quyền'}
                {activeTab === 'deletionHistory' && '👁️ Lịch Sử Xóa Món'}
            </h2>
            <button onClick={onClose} className="hover:bg-rose-600 p-1.5 rounded-lg transition bg-slate-800"><X size={20}/></button>
        </div>
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50 flex flex-col">
          ...
          {activeTab === 'combos' && ( <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 flex flex-col h-full max-w-6xl mx-auto w-full"><div className="flex flex-col gap-3 mb-6 bg-emerald-50 p-4 rounded-xl border border-emerald-200"><div className="flex items-center gap-2 mb-2"><span className="text-sm font-black text-emerald-900 uppercase">1. Chọn Hạng Phòng:</span><select value={currentComboRoomClass} onChange={(e) => setSelectedRoomClassForCombo(e.target.value)} className="w-64 border p-2 rounded-lg text-sm font-bold outline-none cursor-pointer">{roomClasses.map((rc: string) => <option key={rc} value={rc}>{rc}</option>)}</select></div><div className="space-y-2">{draftCombos.map((draft, idx) => (<div key={idx} className="flex gap-2 items-center flex-wrap"><span className="text-xs font-bold text-emerald-700 w-5 text-right">{idx + 1}.</span><input type="text" value={draft.name} onChange={(e)=>handleDraftChange(idx, 'name', e.target.value.toUpperCase())} placeholder="Tên Combo..." className="flex-1 border p-2 rounded-lg text-sm font-bold uppercase min-w-[150px] outline-none" /><div className="flex items-center gap-1 bg-white border rounded-lg px-2"><input type="number" min="0" value={draft.hours} onChange={(e)=>handleDraftChange(idx, 'hours', e.target.value)} placeholder="Giờ" className="w-12 py-2 text-sm font-bold text-center outline-none" /><span className="font-bold">:</span><select value={draft.minutes} onChange={(e)=>handleDraftChange(idx, 'minutes', e.target.value)} className="w-14 py-2 text-sm font-bold outline-none cursor-pointer"><option value="0">00</option><option value="30">30</option></select></div><input type="number" value={draft.price} onChange={(e)=>handleDraftChange(idx, 'price', e.target.value)} placeholder="Giá Ngày Thường" className="w-32 border p-2 rounded-lg text-sm font-bold outline-none" /><input type="number" value={draft.weekendPrice} onChange={(e)=>handleDraftChange(idx, 'weekendPrice', e.target.value)} placeholder="Giá Cuối Tuần" className="w-32 border border-rose-300 bg-rose-50 p-2 rounded-lg text-sm font-bold outline-none" />{draftCombos.length > 1 && (<button onClick={() => handleRemoveDraftRow(idx)} className="p-2 text-rose-500 hover:bg-rose-100 rounded-lg transition"><Trash2 size={16}/></button>)}</div>))}</div><div className="flex justify-between items-center mt-2 pt-4 border-t border-emerald-200"><button onClick={handleAddDraftRow} className="text-emerald-700 text-sm font-bold flex items-center gap-1 bg-emerald-100/50 px-3 py-1.5 rounded-lg transition"><Plus size={16}/> Thêm dòng</button><button onClick={handleSaveBulkCombos} className="bg-emerald-600 text-white px-6 py-2.5 font-bold rounded-lg flex items-center gap-2 hover:bg-emerald-700 transition shadow-sm">Lưu Hàng Loạt Vào Bảng</button></div></div><div className="flex-1 overflow-y-auto border rounded-lg"><table className="w-full text-left border-collapse"><thead className="sticky top-0 bg-slate-100 shadow-sm z-10"><tr className="text-slate-600 text-[11px] uppercase font-black border-b"><th className="p-3">Hạng Phòng</th><th className="p-3">Tên Combo</th><th className="p-3 text-center">Thời Gian (HH:mm)</th><th className="p-3 text-right text-emerald-700">Giá Thường</th><th className="p-3 text-right text-rose-700">Giá Cuối Tuần</th><th className="p-3 text-center w-24">Xóa</th></tr></thead><tbody>{Object.entries(combos).map(([rc, rcCombos]: [string, any]) => ( rcCombos.map((cb: any, idx: number) => { const h = Math.floor(cb.hours); const m = Math.round((cb.hours % 1) * 60); const timeStr = `${h.toString().padStart(2, '0')}:${m.toString().padStart(2, '0')}`; return (<tr key={`${rc}-${cb.name}-${idx}`} className="border-b border-slate-100 hover:bg-emerald-50/50 transition"><td className="p-3 font-bold text-emerald-700 bg-emerald-50/30 w-40">{rc}</td><td className="p-3 font-black text-slate-800">{cb.name}</td><td className="p-3 text-center font-bold text-slate-600 bg-slate-50">{timeStr}</td><td className="p-3 text-right font-black text-emerald-600">{cb.price.toLocaleString()}đ</td><td className="p-3 text-right font-black text-rose-600">{(cb.weekendPrice || cb.price).toLocaleString()}đ</td><td className="p-3 text-center"><button onClick={() => handleDeleteCombo(rc, cb.name)} className="p-2 bg-rose-50 text-rose-600 rounded hover:bg-rose-100 transition"><Trash2 size={16}/></button></td></tr>);}) ))}</tbody></table></div></div> )}
          ...
          {/* TAB MỚI: LỊCH SỬ XÓA MÓN */}
          {activeTab === 'deletionHistory' && ( 
            <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 flex flex-col h-full max-w-5xl mx-auto w-full">
              <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                <div className="p-3 bg-rose-100 text-rose-700 rounded-xl"><History size={26}/></div>
                <div>
                  <h3 className="text-base font-black text-slate-800 uppercase tracking-wider">Theo dõi nhân viên xóa món</h3>
                  <p className="text-[11px] text-slate-500 font-medium mt-1">Ghi lại toàn bộ hành động xóa Combo hoặc Dịch vụ của Lễ tân khỏi Bill của khách.</p>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto border border-slate-200 rounded-lg">
                <table className="w-full text-left border-collapse">
                  <thead className="sticky top-0 bg-slate-100 shadow-sm z-10">
                    <tr className="text-slate-600 text-[10px] uppercase font-black border-b border-slate-200">
                      <th className="p-3 w-40">Thời Gian Xóa</th>
                      <th className="p-3 w-48">Xóa Từ Hóa Đơn (Tên Khách)</th>
                      <th className="p-3 text-center">Phân Loại</th>
                      <th className="p-3">Tên Món Bị Xóa</th>
                      <th className="p-3 text-right">Số Tiền Bị Hụt</th>
                    </tr>
                  </thead>
                  <tbody>
                    {deletionLogs.length === 0 ? (
                      <tr><td colSpan={5} className="p-8 text-center text-slate-400 font-bold italic">Chưa có lịch sử xóa món nào.</td></tr>
                    ) : (
                      deletionLogs.map((log: any) => (
                        <tr key={log.id} className="border-b border-slate-100 hover:bg-rose-50/50 transition">
                          <td className="p-3 font-bold text-slate-600 text-xs">{new Date(log.created_at).toLocaleString('vi-VN')}</td>
                          <td className="p-3 font-black text-slate-800 uppercase">{log.booking_name}</td>
                          <td className="p-3 text-center">
                            <span className={`px-2 py-1 rounded text-[9px] font-black uppercase border ${log.action_type === 'XOÁ COMBO' ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
                              {log.action_type}
                            </span>
                          </td>
                          <td className="p-3 font-bold text-rose-700">{log.item_name}</td>
                          <td className="p-3 text-right font-black text-rose-600">-{log.price.toLocaleString()}đ</td>
                        </tr>
                      ))
                    )}
                  </tbody>
